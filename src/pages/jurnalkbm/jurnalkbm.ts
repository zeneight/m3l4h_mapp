...
@IonicPage()
@Component({
  selector: 'page-jurnalkbm',
  templateUrl: 'jurnalkbm.html',
})
export class JurnalkbmPage {
  tabBarElement: any;
  // nama_mk: string;
  // hari: string;
  // jam: string;
  ...
  getRemoteData(){
    ...
    this.storage.get('resposeData').then((resp) => {
      if(resp !== null){
        ...
        this.http.get('http://153.92.4.64/api/jurnal/'+resposeData.id).map(res => res.json()).subscribe(data => {
